...
class PixelCat {
  ...
  async loadSettings() {
    try {
      const result = await chrome.storage.sync.get(['catActive', 'followMouse', 'catSpeed', 'catSkin', 'catSize']);
  ...
  setupMessageListener() {
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.action === 'updateSettings') {
        // 고양이 활성화/비활성화 처리
        if (typeof request.catActive === 'boolean' && request.catActive !== this.catActive) {
          this.catActive = request.catActive;
          this.updateCatVisibility();
          console.log('🐱 고양이 활성화 상태 변경:', this.catActive);
        }
        
        this.followMouse = request.followMouse;
        this.speed = request.speed;
        
        // 스킨이 변경되었는지 확인
        if (request.skin && request.skin !== this.currentSkin) {
          this.currentSkin = request.skin;
          this.updateCatSkin();
          console.log('🎨 스킨 변경:', this.currentSkin);
        }
        if (typeof request.size === 'number' && request.size !== this.size) {
          this.size = request.size;
          this.updateCatSize();
          console.log('📏 크기 변경:', this.size);
        }
        
        console.log('📨 설정 업데이트:', request);
  ...
  async verifyLicenseKey(licenseKey) {
    ...
    const validLicenses = [
      ...
      'CATPRO2024', 
      'UNLOCK456',
      'PAYPAL_SUCCESS_' + new Date().toISOString().slice(0, 10), // 오늘 날짜 기반
      'TOSS_SUCCESS_' + new Date().toISOString().slice(0, 10)
    ];
    
    return validLicenses.includes(licenseKey.toUpperCase()) || 
           licenseKey.startsWith('PAYPAL_SUCCESS_') || 
           licenseKey.startsWith('TOSS_SUCCESS_');
  }
  ...
  startAnimation() {
    const animate = () => {
      if (!this.isActive || !this.cat || !this.catActive) return;
